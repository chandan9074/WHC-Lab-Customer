...
const MegaMenu = ({ setOpen, open }) => {
    return (
        <div className="fixed top-0 left-0 w-full h-screen bg-red-500 z-50 py-9">
                    ...
                    <div className="col-span-8 flex flex-col">
                        {megaMenuMainLinks.map((item) => (
                            <Link
                                href={item.path}
                                key={item._id}
                                className="text-white hover:text-brand-blue-500 hover:ml-1 text-[90px] font-bold leading-[130px] duration-300"
                            >
                                {item.name}
                            </Link>
                        ))}
